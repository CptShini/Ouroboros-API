    ...
    public static class Playlists
    {
        ...
        public static void GenerateSnipeTimePlaylists(Player sniper, float minAcc, bool local)
        {
            ...
            foreach (Player player in filteredPlayers)
            {
                ...
                if (GenerateSnipeTimePlaylist(player, sniper, local, sniperScores)) i++;
                if (i >= 5) break;
            }
        ...
        public static bool GenerateSnipeTimePlaylist(Player snipedPlayer, Player sniperPlayer, bool local, PlayerScore[] sniperScores)
        {
            string name = snipedPlayer.name;
            string title = $"{name} Snipe Time (#{(local ? snipedPlayer.countryRank : snipedPlayer.rank)})";
            string path = @"Sniping\";
        ...
        public static void GenerateReqPlaylists(Player player, int minStars, int maxStars)
        {
            ...
            for (int i = minStars; i < MathF.Min(maxStars, 12); i++)
            {
                ...
                {
                    GenerateBestRankPlaylist(starScores, stars);
        ...
        public static void GenerateBestRankPlaylist(PlayerScore[] playerScores, StarRange stars)
        {
            playerScores = playerScores.OrderBy(ps => ps.score.rank).ToArray();

            int n = NumResolve(10, playerScores.Length);

            string title = $"{stars.name} highest rank";
